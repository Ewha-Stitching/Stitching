...
%     J. Histochem. Cytochem. 25 (7): 741?3, )
...
%   To simplify, shift x axis by a (bin number axis)
    ...
    height=h-min(lehisto(a:b));
    diagonal=sqrt(height*height+(b-a)*(b-a));
    line=height/(b-a)*x1+min(lehisto(a:b));
    
    temp_h=line-lehisto(x1+a);
    distance =temp_h/diagonal*(b-a);

%   Obtain threshold as the location of maximum L.    
    level=find(max(distance)==distance);
    level=a+mean(level);
